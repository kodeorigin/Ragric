...
const MechanizationWelcome = () => {
  const navigation = useNavigation(); 
  const [mechanizationemail, setMechanizationEmail] = useState("");

  useEffect(() => {
  const getData = async () => {
    try {
      const value = await AsyncStorage.getItem('memail')
      if(value !== null) {
       setMechanizationEmail(value)
      }else{
 ...
 <TouchableHighlight 
            ...
            onPress={() => navigation.navigate("Mechanizationdashboard")}
...
export default MechanizationWelcome
